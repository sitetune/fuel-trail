...
export default async function ReviewReceiptPage({
  ...
  if (!driverCanEditReceipt(receipt.status as ReceiptStatus)) notFound();
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold">{receipt.status === "rejected" ? "Correct and resubmit" : "Review receipt"}</h1>
      <ReceiptReviewForm
